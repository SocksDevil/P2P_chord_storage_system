...
public class Chord {
    ...
    public void retrieveOwnedChunks(SocketAddress peer) {
        final TransferChunksResponse transferChunksResponse = MessageListener.sendMessage(new TransferChunksRequest(this.self.get().getPeerID()), peer);
    ...
    public boolean notify(SocketAddress newPred) {
        ...
        if (this.predecessor.get() == null || this.betweenTwoKeys(this.predecessor.get().getPeerID(), this.self.get().getPeerID(),
            ...
            if (!initialized) {
                initialized = true;
                this.retrieveOwnedChunks(predecessor.get());
                this.retrieveOwnedChunks(successorList.get(0));
            }
